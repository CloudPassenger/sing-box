package proxyproto

import (
	"bufio"
	"errors"
	"net"

	"github.com/sagernet/sing/common/buf"
	singBufio "github.com/sagernet/sing/common/bufio"
	M "github.com/sagernet/sing/common/metadata"

	proxyproto "github.com/pires/go-proxyproto"
)

type Listener struct {
	net.Listener
	AcceptNoHeader bool
}

func (l *Listener) Accept() (net.Conn, error) {
	conn, err := l.Listener.Accept()
	if err != nil {
		return nil, err
	}
	bufReader := bufio.NewReader(conn)
	header, err := proxyproto.Read(bufReader)
	if err != nil {
		if !(l.AcceptNoHeader && errors.Is(err, proxyproto.ErrNoProxyProtocol)) {
			conn.Close()
			return nil, &Error{err}
		}
	}
	if buffered := bufReader.Buffered(); buffered > 0 {
		cache := buf.NewSize(buffered)
		_, err = cache.ReadFullFrom(bufReader, cache.FreeLen())
		if err != nil {
			conn.Close()
			return nil, &Error{err}
		}
		conn = singBufio.NewCachedConn(conn, cache)
	}
	if header != nil {
		return &singBufio.AddrConn{
			Conn:        conn,
			Source:      M.SocksaddrFromNet(header.SourceAddr).Unwrap(),
			Destination: M.SocksaddrFromNet(header.DestinationAddr).Unwrap(),
		}, nil
	}
	return conn, nil
}

var _ net.Error = (*Error)(nil)

type Error struct {
	error
}

func (e *Error) Unwrap() error {
	return e.error
}

func (e *Error) Timeout() bool {
	return false
}

func (e *Error) Temporary() bool {
	return true
}
