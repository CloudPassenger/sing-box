...
func NewInbound(ctx context.Context, router adapter.Router, logger log.ContextLogger, tag string, options option.VLESSInboundOptions) (adapter.Inbound, error) {
	...
	service, err := vless.NewService[adapter.UserID](ctx, options.Decryption, logger, adapter.NewUpstreamContextHandler(inbound.newConnectionEx, inbound.newPacketConnectionEx))
	if err != nil {
		return nil, err
	}
