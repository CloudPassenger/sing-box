...
type Inbound struct {
	...
	service                  *trojan.Service[adapter.UserID]
	userManager              *usermanager.Manager[option.TrojanUser]
...
func NewInbound(ctx context.Context, router adapter.Router, logger log.ContextLogger, tag string, options option.TrojanInboundOptions) (adapter.Inbound, error) {
	...
	service := trojan.NewService[adapter.UserID](adapter.NewUpstreamContextHandlerEx(inbound.newConnection, inbound.newPacketConnection), fallbackHandler, logger)
